import { useState, useEffect } from 'react';
import './Dashboard.css';

export function AdminDashboard() {
  const [stats, setStats] = useState({
    totalProjects: 0,
    totalUsers: 0,
    pendingApprovals: 0,
    thisMonthHours: 0,
  });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // TODO: Fetch real data from API
    setTimeout(() => {
      setStats({
        totalProjects: 12,
        totalUsers: 45,
        pendingApprovals: 8,
        thisMonthHours: 1240,
      });
      setLoading(false);
    }, 500);
  }, []);

  if (loading) {
    return <div className="dashboard-loading">Loading...</div>;
  }

  return (
    <div className="dashboard">
      <h1 className="dashboard-title">Admin Dashboard</h1>
      <p className="dashboard-subtitle">Company Overview</p>

      <div className="dashboard-cards">
        <div className="dashboard-card">
          <div className="dashboard-card-label">Total Projects</div>
          <div className="dashboard-card-value">{stats.totalProjects}</div>
        </div>
        <div className="dashboard-card">
          <div className="dashboard-card-label">Total Users</div>
          <div className="dashboard-card-value">{stats.totalUsers}</div>
        </div>
        <div className="dashboard-card">
          <div className="dashboard-card-label">Pending Approvals</div>
          <div className="dashboard-card-value">{stats.pendingApprovals}</div>
        </div>
        <div className="dashboard-card">
          <div className="dashboard-card-label">This Month Total Hours</div>
          <div className="dashboard-card-value">{stats.thisMonthHours}</div>
        </div>
      </div>

      <div className="dashboard-section">
        <h2>Recent Projects</h2>
        <div className="dashboard-list">
          <div className="dashboard-empty">No projects yet</div>
        </div>
      </div>

      <div className="dashboard-section">
        <h2>Pending Timesheets</h2>
        <div className="dashboard-list">
          <div className="dashboard-empty">No pending timesheets</div>
        </div>
      </div>
    </div>
  );
}

